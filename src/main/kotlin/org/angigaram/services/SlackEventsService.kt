...
@Service
class SlackEventsService(val badgeTransactionsService: BadgeTransactionsService,
    ...
    @Transactional
    fun process(eventId: String, fromSlackUserId: String, slackMessage: String) {
        ...
        if (userMentionMatcher.find()) {
            ...
            badgeTransactionsService.process(eventId, fromUser, toUser, emojis)
        }
    }
    ...
    private fun findAndUpdate(slackUserId: String): User? {
        ...
        val user = User(slackUserJsonNode["user"]["id"].asText(),
                slackUserJsonNode["user"]["profile"]["real_name"].asText(),
                slackUserJsonNode["user"]["tz"].asText(),
                slackUserJsonNode["user"]["profile"]["image_24"].asText())
